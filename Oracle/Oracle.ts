...
export class ZapOracle {
	...
	async initialize() {
		...
		if (title.length == 0) {
			...
			const res: string = await this.oracle.initiateProvider({
				title: Config.title,
		...
		if (!curveSet) {
			...
			if (endpoint.broker == '') endpoint.broker = '0x0000000000000000000000000000000000000000';
			const createEndpoint = await this.oracle.initiateProviderCurve({
			...
			const txid = await this.oracle.setEndpointParams({
				endpoint: endpoint.name,
			...
			ipfs.addJSON(ipfs_endpoint, (err: any, res: any) => {
				...
				this.oracle
					.setProviderParameter({ key: `${endpoint.name}.json`, value: IPFS_GATEWAY + res })
					.then((txid: string) => {
						console.log('saved endpoint info to param with hash : ', res, txid);
					});
			});
			...
			if (endpoint.md && endpoint.md != '') {
				ipfs.add(endpoint.md, (err: any, res: any) => {
					...
					this.oracle
						.setProviderParameter({ key: `${endpoint.name}.md`, value: IPFS_GATEWAY + res })
						.then((txid: string) => {
							console.log('saved endpoint info to param with hash : ', res, txid);
						});
				});
	...
	async handleQuery(queryEvent: any): Promise<void> {
		...
		console.log(
			`Received query to ${event.endpoint} from ${event.onchainSub
				? 'contract'
				: 'offchain subscriber'} at address ${event.subscriber}`
		);
		console.log(
			`Query ID ${event.queryId.substring(0, 8)}...: "${event.query}". Parameters: ${event.endpointParams}`
		);
